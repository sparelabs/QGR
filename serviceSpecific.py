...
def get_requestKPI_service(fromT, toT, APIkey, serviceId):
    '''
    ...
    params2 = {'fromTs': f'{fromT}', 'toTs': f'{toT}', 'metricTypes[]': ['request'], 'serviceIds[]': [serviceId]}
    params3 = {'fromTs': f'{fromT}', 'toTs': f'{toT}', 'metricTypes[]': ['onTimePerformance'], 'serviceIds[]': [serviceId]}
    params4 = {'fromTs': f'{fromT}', 'toTs': f'{toT}', 'metricTypes[]': ['duty'], 'serviceIds[]': [serviceId]}
    params5 = {'fromTs': f'{fromT}', 'toTs': f'{toT}', 'metricTypes[]': ['allTimeRequest'], 'serviceIds[]': [serviceId]}
    params6 = {'fromTs': f'{fromT}', 'toTs': f'{toT}', 'metricTypes[]': ['review'], 'serviceIds[]': [serviceId]}
...
def generateQuarter_service(date, APIkey, serviceId):
    '''
    generateQuarter_service will generate the individual dataframe which holds KPI data per week, and combine them to form
    the total data for the entire quarter
    
    Returns: Dataframe with 13 weeks of entries that encompases almost all of the KPI's created through use of our technology. We selected KPI's
    most usefull to the Partner Success team
    
    Arguments: Start date of the quarter you are interested in, the organization APIkey, and the serviceId
    ...
    for weeks in range(13):
        ...
        weekData = get_requestKPI_service(fromT, toT, APIkey, serviceId)
...
def generateServiceData(date, APIkey):
    '''
    generateServiceData will generate a dataframe consisting of 13 weeks of KPI data that is segregated by service within the organization
    
    '''
    #First get a list of service Id's
    services = callfor_services(APIkey)
    services = list(services['id'])
    
    dataStore = []
    for e in services:
        #get data based on service ID
        data = generateQuarter_service(date, APIkey, e)
